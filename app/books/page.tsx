...
// ── Book cover: Cloudinary img or gradient fallback ─────────────────────────
function BookCover({
  coverUrl,
  title,
  className,
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {categoryId && categories.find((c) => String(c.id) === categoryId) && (
                    <Badge variant="default" className="gap-1 cursor-pointer" onClick={() => { setCategoryId(""); setPage(1); }}>
              ...
              {isLoading ? (
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
                  {books.map((book, idx) => (
                    <Link href={`/books/${book.id}`} key={book.id} className="group opacity-0 animate-[heroReveal_0.5s_ease_forwards]" style={{ animationDelay: `${idx * 0.05}s` }}>
                ...
                <div className="space-y-3">
                  {books.map((book, idx) => (
                    <Link href={`/books/${book.id}`} key={book.id} className="group block opacity-0 animate-[heroReveal_0.4s_ease_forwards]" style={{ animationDelay: `${idx * 0.04}s` }}>
