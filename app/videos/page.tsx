...
function SkeletonCard() {
  return (
    <div className="rounded-2xl border border-purple-100/60 dark:border-gray-800/60 overflow-hidden bg-white/80 dark:bg-gray-900/80">
...
function VideosPageContent() {
  const {
    ...
    setPage,
    listingHref,
  } = useCatalogUrlState(sortOptions);
  ...
  const filtered = books.filter((b) => b.videoUrl);
  ...
  return (
    <>
      {/* ── Hero Banner ── */}
      <section className="relative overflow-hidden bg-gradient-to-b from-purple-900 via-purple-800 to-indigo-900 py-20 sm:py-24">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-[-30%] left-1/2 -translate-x-1/2 w-[700px] h-[700px] rounded-full bg-purple-400/10 blur-[120px]" />
          <div className="absolute bottom-[-20%] right-[-10%] w-[400px] h-[400px] rounded-full bg-indigo-400/10 blur-[100px]" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white leading-[1.1] tracking-tight mb-4">
            Watch &{" "}
            <span className="bg-gradient-to-r from-purple-300 via-violet-200 to-purple-300 bg-clip-text text-transparent">
              Learn
            </span>
          </h1>
          <p className="text-base sm:text-lg text-white/55 max-w-xl mx-auto mb-8">
            Supplementary video materials to support your academic journey.
          </p>
          ...
          <div className="max-w-lg mx-auto">
            <div className="flex gap-2 bg-white/[0.08] backdrop-blur-md border border-white/[0.12] rounded-2xl p-1.5">
              ...
              <Button
                size="lg"
                className="bg-purple-600 hover:bg-purple-500 text-white rounded-xl px-6 h-11 font-semibold"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            >
              ...
              {/* Categories */}
              <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-2xl border border-purple-100/60 dark:border-gray-800/60 p-5 shadow-sm">
                  ...
                  <li>
                    <button
                      ...
                      className={cn("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors",
                        !categoryId ? "bg-purple-600 text-white font-semibold" : "text-[#5E5E5E] dark:text-gray-400 hover:bg-purple-600/10 hover:text-purple-700"
                      )}
                    >All</button>
                  </li>
                  {categories.map((cat) => (
                    <li key={cat.id}>
                      <button
                        ...
                        className={cn("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors",
                          categoryId === String(cat.id) ? "bg-purple-600 text-white font-semibold" : "text-[#5E5E5E] dark:text-gray-400 hover:bg-purple-600/10 hover:text-purple-700"
                        )}
                      >{cat.name}</button>
                    </li>
                  ))}
                ...
                <div className="space-y-1">
                  {sortOptions.map((opt) => (
                    <button
                      ...
                      onClick={() => setSort(opt)}
                      className={cn("w-full text-left px-3 py-2 rounded-lg text-sm transition-colors",
                        sortOpt === opt ? "bg-purple-600/10 text-purple-700 dark:text-purple-300 font-semibold" : "text-[#5E5E5E] dark:text-gray-400 hover:bg-purple-600/5"
                      )}
                    >{opt.label}</button>
                  ))}
                </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isLoading ? (
                    ...
                    <span className="text-sm text-[#5E5E5E] dark:text-gray-400">
                      <span className="font-semibold text-[#1A1A1A] dark:text-white">{total}</span> video{total !== 1 ? "s" : ""} found
                    </span>
                  )}
                  ...
                  {categoryId && categories.find((c) => String(c.id) === categoryId) && (
                    <Badge variant="default" className="gap-1 cursor-pointer bg-purple-600 hover:bg-purple-700" onClick={() => setCategory("")}>
                ...
                <div className="flex items-center gap-1 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-purple-100/60 dark:border-gray-800/60 rounded-xl p-1 shadow-sm">
                  <button onClick={() => pushQuery({ view: null })} className={cn("p-1.5 rounded-md transition-colors", view === "grid" ? "bg-purple-600 text-white" : "text-[#5E5E5E] hover:text-purple-700")}>
                    <Grid3X3 className="w-4 h-4" />
                  </button>
                  <button onClick={() => pushQuery({ view: "list" })} className={cn("p-1.5 rounded-md transition-colors", view === "list" ? "bg-purple-600 text-white" : "text-[#5E5E5E] hover:text-purple-700")}>
              ...
              {isError && !isLoading && (
                <div className="flex items-center gap-3 p-5 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 mb-6">
                  ...
                  <p className="text-sm">Failed to load videos. Please check your connection.</p>
                </div>
              )}
              ...
              {isLoading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <h3 className="text-xl font-bold text-[#1A1A1A] dark:text-white mb-2">No videos found</h3>
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filtered.map((book, idx) => (
                    <Link
                      href={withCatalogReturnHref(`/books/${book.id}?tab=video`, listingHref)}
                      key={book.id}
                      className="group block opacity-0 animate-[heroReveal_0.5s_ease_forwards]"
                      style={{ animationDelay: `${idx * 0.05}s` }}
                    >
                      <div className="relative h-full">
                        <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-indigo-500 rounded-2xl opacity-0 group-hover:opacity-30 blur-md transition-all duration-500" />
                        <Card className="relative overflow-hidden bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm border-purple-100/60 dark:border-gray-800/60 h-full transition-all duration-500 group-hover:-translate-y-1.5 group-hover:shadow-2xl">
                          {/* Cover */}
                          <div className="relative h-52 overflow-hidden shrink-0">
                            ...
                            <div className="absolute bottom-3 left-3 flex gap-2">
                              <Badge className="bg-purple-600 text-white border-0 text-[10px] font-bold">VIDEO</Badge>
                              {book.Category && (
                                <Badge className="bg-white/90 dark:bg-gray-900/90 text-purple-700 dark:text-purple-300 border-0 text-[10px] font-semibold">
                                  {book.Category.name}
                                </Badge>
                              )}
                            </div>
                            ...
                            <div className="absolute top-3 right-3 flex flex-col gap-1">
                              {(book.views ?? 0) > 0 && (
                                <span className="flex items-center gap-1 text-[10px] font-medium bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm text-[#5E5E5E] dark:text-gray-300 px-2 py-1 rounded-full shadow-sm">
                                  <Eye className="w-3 h-3 text-purple-600" />{book.views}
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <CardContent className="p-5">
                            <h3 className="font-bold text-[#1A1A1A] dark:text-white text-sm leading-snug line-clamp-2 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors duration-300 mb-1.5">
                              {book.title}
                            </h3>
                ...
                <div className="space-y-3">
                  {filtered.map((book, idx) => (
                    ...
                    >
                      <Card className="overflow-hidden bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm border-purple-100/60 dark:border-gray-800/60 transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-0.5">
                        <CardContent className="p-4 sm:p-5 flex gap-4 sm:gap-5 items-center">
                          <div className="w-24 h-16 rounded-xl overflow-hidden shrink-0 shadow-md relative">
                            <VideoCover coverUrl={book.coverUrl ? `/api/books/${book.id}/cover` : null} title={book.title} className="transition-transform duration-500 group-hover:scale-105" />
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              {book.Category && (
                                <span className="inline-flex items-center text-[10px] font-semibold uppercase tracking-wider bg-purple-600/10 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded-full">
                                  {book.Category.name}
                                </span>
                              )}
                            </div>
                            <h3 className="font-bold text-[#1A1A1A] dark:text-white text-sm sm:text-base group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors duration-300 truncate">
                              {book.title}
                            </h3>
                            ...
                            <div className="flex items-center gap-3 mt-1.5">
                              {(book.views ?? 0) > 0 && (
                                <span className="flex items-center gap-1 text-xs text-[#9CA3AF]">
                                  <Eye className="w-3.5 h-3.5 text-purple-500/60" />{book.views}
                                </span>
                              )}
                              {(book.downloads ?? 0) > 0 && (
                                <span className="flex items-center gap-1 text-xs text-[#9CA3AF]">
                                  <Download className="w-3.5 h-3.5 text-indigo-500/60" />{book.downloads}
                                </span>
                              )}
                            </div>
                          </div>
                          <div className="shrink-0 hidden sm:flex items-center justify-center w-9 h-9 rounded-full bg-purple-600/5 dark:bg-purple-600/10 text-purple-600/40 group-hover:text-purple-600 group-hover:bg-purple-600/10 transition-all duration-300">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((item, i) =>
                      item === "…" ? (
                        ...
                        <Button key={item} variant={page === item ? "default" : "outline"} size="sm" className={cn("w-9 h-9 p-0", page === item && "bg-purple-600 hover:bg-purple-700 border-purple-600")} onClick={() => setPage(item as number)}>
                          {item}
                        </Button>
                      )
                    )}
    ...
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center bg-[#F8FAFC] dark:bg-gray-950">
          <Loader2 className="h-8 w-8 animate-spin text-purple-600" aria-label="Loading videos" />
        </div>
      }
    >
      <VideosPageContent />
